...
| Meshy 다운로드 포맷 | **GLB** (Unity에 넣는 포맷이 아니다 — §G.0 참조) |
...
```
Meshy  ──GLB──▶  Blender  ──FBX──▶  Unity  ──▶  WebGL 빌드
       (다운로드)   (정규화·아틀라스·LOD)  (임포트)
```

**GLB로 받고 FBX로 내보낸다.** 이유는 두 가지다.

- Blender의 glTF 임포터가 **PBR 머티리얼을 가장 깔끔하게** 받는다. Albedo/Normal/Metallic/Roughness가
  Principled BSDF에 자동 연결되고 텍스처가 파일 하나에 임베드된다. Meshy의 FBX는 텍스처 연결이 깨지는 경우가 있다.
- **Unity는 glTF/GLB를 기본 지원하지 않는다.** `glTFast` 등 패키지가 필요하다. FBX는 기본 지원이다.

> 빌드 타깃(WebGL)은 임포트 포맷과 **무관**하다. FBX로 넣는 것은 동일하고,
> 플랫폼 대응은 텍스처 압축 설정(DXT)에서만 이루어진다.

### G.1 Meshy에서 목표 폴리로 직접 뽑은 경우

Meshy의 `target_polycount` 로 이미 최종 폴리를 맞췄다면 **Decimate 단계는 건너뛴다.**
다만 Blender 자체를 건너뛸 수는 없다 — 남은 이유는 폴리가 아니라 **텍스처와 원점**이다.

| 작업 | Meshy가 해주나 | 건너뛸 수 있나 |
|---|---|---|
| 폴리 감축 | ✅ target polycount | **가능** |
| 아틀라스 통합 | ❌ | **불가** — 텍스처 495MB, 드로우콜 +51 |
| LOD 체인 | ❌ | 초기엔 가능, M5에서 필수 |
| 원점 바닥 정렬 | ❌ | Unity에서 부모 오브젝트로 우회 가능(비권장) |
| 부착점 (§D) | ❌ | D 등급만 해당 |

### G.2 Blender 작업 절차 (항목마다 반복)

1. **임포트** — `File > Import > glTF 2.0 (.glb/.gltf)`
2. **스케일 정규화** — `N` 사이드바에서 **Dimensions**를 §G.3 실측표 값으로 맞춘 뒤 **`Ctrl+A` > `Scale`**
   > 스케일 적용을 빠뜨리면 Unity에서 Transform Scale이 1이 아닌 채 들어가 **정적 배칭이 깨진다.**
3. **원점을 바닥 중심으로**
   1. `Shift+C` — 3D 커서를 월드 원점으로
   2. Edit Mode(`Tab`) → `A` 전체 선택 → **바닥면이 Z=0에 닿도록** 이동
   3. Object Mode → `Object > Set Origin > Origin to 3D Cursor`
4. **회전 정리** — 정면이 **−Y**를 향하게 한 뒤 `Ctrl+A` > `Rotation`
5. **최종 확인** — `Object > Apply > All Transforms`
6. *(선택)* **Decimate** — Meshy에서 목표 폴리로 뽑았다면 생략. 아니면 Planar 먼저, 부족하면 Collapse
7. *(모아서)* **UV 재배치 + 텍스처 베이크** — 아틀라스. §G.5 참조
8. *(M5)* **LOD 생성** — LOD1 50% / LOD2 20% / LOD3 5%
9. **FBX 익스포트** — `File > Export > FBX`

#### FBX 익스포트 설정

| 항목 | 값 |
|---|---|
| Forward / Up | **`-Z Forward` / `Y Up`** (Unity 축 대응) |
| **Apply Transform** | ✅ **필수** |
| Object Types | Mesh만 |
| Apply Modifiers | ✅ |
| Path Mode | `Copy` + 임베드 |
...
**Model 탭** — Scale Factor `1` / Mesh Compression `Off` / Read·Write `Off` / Normals `Import` / Tangents `Calculate Mikktspace`

**Texture 탭** — Max Size `2048`(아틀라스) · `1024`(C 등급) / Compression `DXT1`(Albedo·ORM) · `DXT5(BC3)`(Normal)
/ sRGB는 **Albedo만 체크**, Normal·ORM은 해제 / Normal은 Texture Type을 `Normal map`으로

> sRGB를 ORM에 켜두면 러프니스가 미묘하게 어긋난다. 눈치채기 어려운 종류의 버그이므로 처음에 잡아둔다.

#### 임포트 후 검증

- Transform Scale이 **(1, 1, 1)** 인가
- 오브젝트가 **바닥에 서 있는가** (파묻히거나 떠 있지 않은가)
- 씬에 **1m 큐브**를 놓고 크기를 대조했는가

### G.3 실측 치수표 (Blender Dimensions 입력값)

단위 **미터**. `W × D × H` = 가로 × 깊이 × 높이. Blender 사이드바(`N`)의 Dimensions에 그대로 넣는다.

**신뢰도**: ◎ 확인된 규격 · ○ 표준 규격 추정 · △ 형태 기준 추정

#### A. 소형 소품

| # | 에셋 | W | D | H | 신뢰 | 비고 |
|---|---|---:|---:|---:|:--:|---|
| A-01 | 서류 바인더 | 0.24 | 0.32 | 0.05 | ○ | A4 규격 기준 |
| A-02 | 걸레 | 0.30 | 0.25 | 0.06 | △ | 접힌 상태 |
| A-03 | 싸리비 | 0.35 | 0.10 | 1.30 | △ | 자루 포함 |
| A-04 | 대걸레 | 0.40 | 0.12 | 1.40 | △ | |
| A-05 | 야전삽 (접이식) | 0.16 | 0.08 | 0.26 | ○ | **접은 상태**. 펴면 전장 0.60 |
| A-06 | 수통 + 컵 | 0.14 | 0.09 | 0.22 | ○ | 1L 수통 + 컵 결합 |
| A-07 | 붕대 롤 | 0.06 | 0.06 | 0.05 | △ | |
| A-08 | 무전기 배터리 | 0.12 | 0.06 | 0.09 | △ | |
| A-09 | 몽키스패너 | 0.06 | 0.02 | 0.25 | △ | |
| A-10 | 드라이버 | 0.03 | 0.03 | 0.22 | △ | |
| A-11 | 6칸 식판 | 0.36 | 0.28 | 0.03 | ○ | 한국 급식 표준 |
| A-12 | 잔반통 | 0.40 | 0.40 | 0.55 | △ | 원통 지름 0.40 |
| A-13 | 분리수거함 | 0.40 | 0.30 | 0.55 | △ | |
| A-14 | K2 탄창 | 0.03 | 0.08 | 0.19 | ○ | 5.56mm 30발 곡선형 |
| A-15 | 총기 손질 세트 | 0.25 | 0.12 | 0.05 | △ | 펼친 상태 |
| A-16 | 약병 | 0.05 | 0.05 | 0.10 | △ | |
| A-17 | 체온계 | 0.03 | 0.02 | 0.14 | △ | |
| A-18 | 야전 손전등 | 0.07 | 0.05 | 0.17 | ○ | L자형 |
| A-19 | 열쇠 꾸러미 | 0.09 | 0.03 | 0.12 | △ | |
| A-20 | 모래주머니 | 0.35 | 0.20 | 0.12 | ○ | 채워진 상태 |
| A-21 | 세탁 바구니 | 0.45 | 0.33 | 0.28 | △ | |
| A-22 | 수건 | 0.25 | 0.18 | 0.04 | △ | 접힌 상태 |
| A-23 | 목장갑 | 0.24 | 0.12 | 0.02 | ○ | 한 켤레 나란히 |
| A-24 | 접힌 태극기 | 0.30 | 0.30 | 0.06 | △ | 삼각 접기 |
| A-25 | 급수통 20L | 0.35 | 0.20 | 0.45 | ○ | |

#### B. 중형 소품

| # | 에셋 | W | D | H | 신뢰 | 비고 |
|---|---|---:|---:|---:|:--:|---|
| B-01 | **관물대** | **0.50** | 0.50 | 1.80 | ◎/△ | **폭 0.50은 확인값**, 깊이·높이는 추정 |
| B-02 | 야전 난로 | 0.50 | 0.50 | 0.90 | △ | **연통 별도** — 지름 0.10, 길이 2.0+ |
| B-03 | 발전기 | 0.65 | 0.45 | 0.55 | △ | |
| B-04 | 무전기 본체 + 원격제어기 | 0.35 | 0.25 | 0.30 | △ | |
| B-05 | 의약품장 | 0.60 | 0.20 | 0.75 | △ | 벽걸이 — 원점은 뒷면 중심 |
| B-06 | 공구 캐비닛 | 0.70 | 0.45 | 1.00 | △ | |
| B-07 | 배식대 | 2.00 | 0.70 | 0.90 | △ | |
| B-08 | 안테나 마스트 | 0.80 | 0.80 | 2.40 | △ | 삼각대 펼친 상태 |
| B-09 | 야전 전화기 | 0.25 | 0.15 | 0.22 | △ | |
| B-10 | 유류 드럼통 200L | 0.58 | 0.58 | 0.88 | ◎ | 200L 드럼 표준 규격 |
| B-11 | 단층 철제 침대 | 1.00 | 2.00 | 0.70 | ○ | |
| B-12 | 창고 선반 유닛 | 1.20 | 0.45 | 1.80 | △ | |
| B-13 | 공용 세면대 | 2.40 | 0.55 | 1.60 | △ | 거울 포함 높이 |
| B-14 | 사격 표적틀 | 0.60 | 0.30 | 1.20 | △ | |
| B-15 | 제독 샤워 스탠드 | 0.80 | 0.80 | 2.20 | △ | |

#### C. 대형 소품

| # | 에셋 | W | D | H | 신뢰 | 비고 |
|---|---|---:|---:|---:|:--:|---|
| C-01 | K-511 (카고) | 2.40 | 6.70 | 2.70 | ○ | **깊이가 전장** — 차량은 −Y가 정면 |
| C-02 | K-511 (호로) | 2.40 | 6.70 | 3.00 | ○ | 호로 높이 반영 |
| C-03 | 야전 텐트 | 5.00 | 4.50 | 2.80 | △ | 분대 텐트 기준 |
| C-04 | 들것 | 0.55 | 2.20 | 0.15 | ○ | 펼친 상태 |
| C-05 | 급수 탱크 트레일러 | 1.80 | 3.50 | 1.90 | △ | 견인바 포함 |

#### D. 지급 장비 · E. 부록

| # | 에셋 | W | D | H | 신뢰 | 비고 |
|---|---|---:|---:|---:|:--:|---|
| D-01 | **K2 소총** | 0.07 | **0.98** | 0.26 | ◎ | **전장 0.98** / 개머리판 접으면 0.73 |
| D-02 | PRC-999K 배낭 | 0.35 | 0.20 | 0.50 | △ | 안테나 제외 |
| D-03 | 의무낭 | 0.35 | 0.15 | 0.25 | △ | |
| D-04 | 휴대 공구 가방 | 0.40 | 0.20 | 0.28 | △ | |
| D-05 | 야간투시경 | 0.12 | 0.10 | 0.08 | △ | |
| D-06 | 클립보드 | 0.23 | 0.32 | 0.02 | ○ | A4 규격 |
| E-01 | K-5 방독면 | 0.22 | 0.25 | 0.28 | △ | 정화통 2개 포함 |
| E-02 | 방탄모 | 0.24 | 0.28 | 0.20 | △ | |

> **△ 항목은 실측이 아니다.** 형태가 성립하는 범위의 추정치이므로, 실물 사진이나 규격서를 확보하면 갱신한다.
> 특히 B-01 관물대는 폭만 확인됐다(§J-1).

### G.4 원점 배치 예외

기본은 **바닥 중심**이지만 아래는 다르다.

| 유형 | 원점 위치 | 해당 |
|---|---|---|
| 벽걸이 | 뒷면 중심 | B-05 의약품장, B-09 야전 전화기, B-13 세면대 |
| 손 부착물 | 그립 중심 | D-01, D-04, D-06, A-05, A-09, A-10, A-18 |
| 등·허리 부착물 | 접촉면 중심 | D-02, D-03 |
| 두부 부착물 | 머리 중심 | D-05, E-01, E-02 |
| 차량 | 바닥 중심, **−Y가 정면** | C-01, C-02, C-05 |

### G.5 아틀라스 작업 시점

**개별 항목마다 하지 않는다.** 등급별로 모아서 한 번에 처리한다.

| 시점 | 대상 | 근거 |
|---|---|---|
| B 등급 15종 완료 시 | `ATLAS_B1` / `B2` | 첫 아틀라스. 여기서 파이프라인을 확립하면 나머지는 반복 |
| D + E 완료 시 | `ATLAS_C` | 부착점 정리와 함께 |
| A 등급 25종 완료 시 | `ATLAS_A` | 물량이 가장 많음 |
| C 등급 | 없음 | 개별 1024 텍스처 유지 |

> **M0 단계에서는 아틀라스를 건너뛰어도 된다.** 맵 1개에 소품 몇 개뿐이라 텍스처 예산이 문제되지 않는다.
> 소품 45종이 전부 들어가는 **M3부터는 필수**다.
