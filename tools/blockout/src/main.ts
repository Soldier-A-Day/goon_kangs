...
const TARGETS: readonly Target[] = [
  { id: "equip.rifle", build: buildRifle },
  {
    id: "base.drillGround",
    build: (detail) => assemble(DRILL_GROUND, detail),
    moduleCount: DRILL_GROUND.length,
  },
  {
    id: "base.barracks",
    build: (detail) => assemble(BARRACKS, detail),
...
function main(): void {
  ...
  for (const target of TARGETS) {
    ...
    const { mesh, detail } = fitToBudget(budget, target.build);
    ...
    writeFileSync(
      ...
      toObj(
        mesh,
        `${entry.label} — 블록아웃 (예산 ${budget.toLocaleString()} tris · 디테일 ${detail})`,
      ),
    );

    const fill = (mesh.triangleCount / budget) * 100;
    const modules = target.moduleCount ? ` · 모듈 ${target.moduleCount}종` : "";
    console.log(
      `✓ ${entry.id.padEnd(18)} ${mesh.triangleCount.toLocaleString().padStart(7)} tris / ` +
        `${budget.toLocaleString().padStart(7)} (${fill.toFixed(0)}%)${modules}`,
    );
  }
