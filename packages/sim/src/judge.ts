 ...
 * 셋을 동시에 만족시킬 수 없다.
 *
 * 상위 목표인 완주율을 지키는 쪽으로 **확정했다** — 70%를 조건 A에 적용하면
 * 실측 완주율이 15% → 31%로 뛰어 목표 구간을 벗어난다(tools/simrunner).
 * 70% 규칙은 훈련 자체의 성패(군기·보상)에만 쓴다.
 *
 * 감수하는 대가: 체크포인트가 100% 강제되므로 표 13-1의 "훈련 100% 완주 +8"이
 * 자동 획득이 되어 변별력을 잃는다. 그 결과 선택 퀘스트를 하나도 하지 않아도
 * D-14 재심사에서 병장에 닿는다(test/ranks.test.ts에 고정). 승급이 후한 것은
 * 완주율이 흔들리는 것보다 낫다는 판단이다.
 */
